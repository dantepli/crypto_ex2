    ...
    for test in tests:
        ...
        for i in range(len(test["ops"])):
            ...
            if op == 'r':
                ...
                res = cloud.Read(pos)
                if not expected == res:
                    print "!! error: reading position %d, expected \\x%s but received \\x%s " % (
                        pos, "{:02x}".format(ord(expected)), "{:02x}".format(ord(res)))
                    print "failed..."
                    exit(1)
                ...
                if not expected == res:
                    print "!! error: writing value \\x%s to position %d, expected \\x%s but received \\x%s " % (
                        "{:02x}".format(ord(value)), pos, "{:02x}".format(ord(expected)), "{:02x}".format(ord(res)))
                    print "failed..."
                    exit(1)
